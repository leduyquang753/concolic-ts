...
export function calculateEmployeeNationalInsurance(country: Country, taxableAnnualIncome: number): number {
  const weeklySalary = taxableAnnualIncome / 52;
  const { NI_MIDDLE_RATE, NI_UPPER_RATE, NI_MIDDLE_BRACKET, NI_UPPER_BRACKET } = getHmrcRates(country);
