...
export function calculatePensionAnnualAllowance(
  totalAnnualIncome: number, retrospectivePensionPaymentsTaxRelief: number,
  employeeDcPensionContributions: number, employerDcPensionContributions: number, lumpSumDeathBenefits: number
) {
  const {
    PENSION_ANNUAL_ALLOWANCE, // 60k
    PENSION_MINIMUM_ANNUAL_ALLOWANCE, // 10k
    PENSION_ADJUSTED_LIMIT, // 260k
  } = getHmrcRates("England/NI/Wales");
  ...
  const reduction = amountOver > 0 ? amountOver / 2 : 0;
